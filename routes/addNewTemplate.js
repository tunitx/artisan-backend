...
router.post('/addNewTemplate', verifyToken, parser.single('template'), async (req, res) => {
  ...
  try {
    cloudinary.uploader.upload_stream({ resource_type: 'raw', folder: 'Templates' }, async (error, result) => {
      if (error) {
...
console.log(result.public_id);
      ...
      res.status(200).json({ message: 'template created', cloudinaryUrl, template })
